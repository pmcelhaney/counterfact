...
function buildApplyContextContent(contextFiles: ContextFileInfo[]): string {
  ...
  const parts = [
    ...
    "",
    "/** Base class for Context objects defined in _.context.ts files */",
    ...
    "",
    '  constructor({ loadContext, readJson }: Pick<ApplyContext, "loadContext"> & {',
    "    readJson: (relativePath: string) => Promise<unknown>;",
    "  }) {",
    "    Object.assign(this, { loadContext, readJson });",
    "  }",
    "}",
    "",
  ];
